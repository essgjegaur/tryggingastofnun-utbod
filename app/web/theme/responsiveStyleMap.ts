...
export const resolveBreakpoints = (variant: any, attr: any, acc: any) => {
  if (isObject(variant[attr])) {
    ;(Object.keys(variant[attr]) as Breakpoint[]).reduce((acc, breakpointKey) => {
      ...
      if (breakpointKey === 'xs') {
        ...
        if (!acc['@media'][`(min-width: ${theme.breakpoints[breakpointKey]}px)`]) {
...
) => style(Object.keys(styleObj).reduce((acc, attr) => resolveBreakpoints(styleObj, attr, acc), {}))
